...
- `~/.config/ai-profiles/*.yaml` + the `_ai_profile` zsh resolver are gone. Profile selection is now just mise `[env]`: `CLAUDE_CONFIG_DIR`/`PI_CONFIG_DIR` default to the personal profiles in `mise/conf.d`, and `~/workspace/REDACTEDengineering/mise.toml` overrides them per-root. mise already walks the directory hierarchy — no reason to reimplement that in shell
