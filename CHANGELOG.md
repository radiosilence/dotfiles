...
- All 39 relative `source "${0:A:h:h}/..."` lines are gone. A script now says what it wants and nothing about where it lives: `autoload -Uz zarg_init _dt_head _dt_info`. One name covers zarg — `zarg_init` is always the first call, so it declares the rest of the API
...
- zarg had to survive being autoloaded without its loader: `zarg_init` now declares the state the rest of the library reads, and `zarg_version` finds the repo through `$functions_source` rather than a `ZARG_HOME` set at source time. Every function declares its own private helpers, so autoloading the public names is enough
